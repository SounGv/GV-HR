...
function ItemCard({ item, onDelete }: { item: DevelopmentItem; onDelete: () => void }) {
  ...
  const [employeeNote, setEmployeeNote] = useState(item.employeeNote ?? "");
  ...
  async function saveEmployeeNote() {
    setSavingNote(true);
    try {
      await updateMut.mutateAsync({ itemId: item.id, input: { employeeNote } });
  ...
  return (
    <Card className="p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          {item.problemDescription && (
            <p className="mt-0.5 text-sm text-muted-foreground">สาเหตุ: {item.problemDescription}</p>
          )}
          <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
            ...
            {item.followUpDate && (
              <span className="flex items-center gap-1">
                <CalendarDays className="size-3.5" /> ติดตาม {formatDate(item.followUpDate)}
              </span>
            )}
          </div>
          {item.managerNote && (
            <p className="mt-2 rounded-lg bg-muted/50 p-2 text-xs text-muted-foreground">
              <span className="font-medium text-foreground">หมายเหตุหัวหน้า:</span> {item.managerNote}
            </p>
          )}
        </div>
      ...
      <div className="mt-3 space-y-1.5 border-t border-border pt-3">
        <label className="text-xs font-medium text-muted-foreground">ความคิดเห็นของฉัน</label>
        <div className="flex items-center gap-2">
          <Input value={employeeNote} onChange={(e) => setEmployeeNote(e.target.value)} placeholder="แสดงความคิดเห็นต่อแผนพัฒนานี้..." className="h-8 text-xs" />
          <Button size="sm" variant="outline" onClick={saveEmployeeNote} disabled={savingNote || employeeNote === (item.employeeNote ?? "")}>
            บันทึก
          </Button>
        </div>
      </div>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-3">
          ...
          <Textarea placeholder="สาเหตุ (ถ้ามี)" rows={2} value={problemDescription} onChange={(e) => setProblemDescription(e.target.value)} />
